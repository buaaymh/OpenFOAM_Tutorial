...
Description
    This is the OpenFOAM CodeStream tutorial from buaaymh.
...
int main(int argc, char *argv[])
{
    ...
    Info << "\nPatch movingWall:" << endl;
    word findAPatch("movingWall");
    label patchNum = mesh.boundary().findPatchID(findAPatch);
    const vectorField& U_patch = U.boundaryField()[patchNum];
    forAll(U_patch, faceI)
    {
        Info << U_patch[faceI] << endl;
    }
    Info << endl;

    Info << "Time now = " << runTime.timeName() << endl << endl;

    Info << "Fields have been written." << endl << endl;
